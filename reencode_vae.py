...
class BigConvVAE(nn.Module):
    """
    Deeper and wider VAE (from train_big_vae.py):
...
def reencode_files(
    ...
    # Detect which model architecture to use based on checkpoint keys
    state_dict_keys = ckpt["model_state_dict"].keys()
    is_big_vae = any("encoder_stages" in key for key in state_dict_keys)

    # Build model and transforms first
    if is_big_vae:
        print(f"Detected BigConvVAE architecture (from train_big_vae.py)")
        model = BigConvVAE(in_channels=1, latent_dim=latent_dim, beta=beta, dropout=0.0).to(device)
    else:
        print(f"Detected ConvVAE architecture")
