#!/usr/bin/env python3
"""
ML-based Audio Denoising Script using Resemble-Enhance

Applies deep learning-based denoising to audio files using Resemble-Enhance.
...
try:
    ...
    print("Error: Resemble-Enhance not installed. Please install it with:", file=sys.stderr)
    print("  pip install resemble-enhance", file=sys.stderr)
    print("\nOr with uv:", file=sys.stderr)
    print("  uv add resemble-enhance", file=sys.stderr)
    sys.exit(1)
...
class ResembleAudioDenoiser:
    def __init__(
        self,
        directory: Path,
        num_workers: int = 1,  # Resemble-Enhance is GPU-intensive, default to 1
        progress_file: str = '.denoise_resemble_progress.json',
    ...
    def _process_file(self, file_path: Path) -> Tuple[bool, str, Optional[str]]:
        """
        Process a single audio file with Resemble-Enhance.
        ...
        try:
            ...
            # Process using Resemble-Enhance
            ...
            # Resemble-Enhance expects shape (1, samples)
            if audio.dim() == 1:
                audio = audio.unsqueeze(0)

            # Apply enhancement
            enhanced_audio, output_sr = enhance(
                audio,
                sr,
                self.device,
                nfe=64,  # Number of function evaluations, higher = better quality but slower
                solver="midpoint",
                lambd=0.5,  # Balance between denoising and enhancement
                tau=0.5,
            )

            # Resample to target sample rate if specified
            if self.sample_rate and output_sr != self.sample_rate:
    ...
    def process_all(self):
        ...
        self.logger.info(f"Starting processing with Resemble-Enhance")
        mode = "Denoise only" if self.denoise_only else "Denoise + Enhance"
        self.logger.info(f"Mode: {mode}")

        if self.num_workers > 1:
            self.logger.warning(
                f"Note: Resemble-Enhance is GPU/CPU intensive. "
        ...
        # Process files sequentially (ML models don't parallelize well)
...
def main():
    parser = argparse.ArgumentParser(
        description='Denoise audio files using Resemble-Enhance ML model',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Resemble-Enhance:
  - Two-stage AI model: denoiser + enhancer
  - Trained on high-quality 44.1kHz speech data
  - Excellent for speech and vocalizations (including bird calls)
  - Removes noise, distortions, and bandwidth limitations
...
Examples:
  ...
  %(prog)s /path/to/audio --denoise-only  # Skip enhancement stage
  ...
  %(prog)s /path/to/audio --no-gpu --dry-run
    ...
    model_group.add_argument(
        '--denoise-only',
        action='store_true',
        help='Only denoise, skip enhancement stage (faster, less processing)'
    )
    ...
    process_group.add_argument(
        '--progress-file', '-p',
        type=str,
        default='.denoise_resemble_progress.json',
        help='Progress tracking file name (default: .denoise_resemble_progress.json)'
    )
    ...
    try:
        denoiser = ResembleAudioDenoiser(
